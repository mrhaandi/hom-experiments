...
\end{equation}
where the metavariable $X$ has simple type $\varphi_1 \to \cdots \to \varphi_n \to \varphi$, each $u_i^j$ has simple type $\varphi_j$, each $r_i$ is a simply-typed $\beta\eta$-normal form of type $\varphi$, and free variables are taken from a fixed simply-typed context $\Phi$.
...
A faithful translation of game positions into system~\textbf{R} judgments could provide an independent type-theoretic justification of Stirling's argument.
