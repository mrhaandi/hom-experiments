...
\author{Aleksy Schubert}{University of Warsaw, Poland}{alx@mimuw.edu.pl}{https://orcid.org/0000-0002-9316-6098}{}{}

\author{Jakob Rehof}{TU Dortmund University and Lamarr Institute for ML and AI, Germany}{jakob.rehof@cs.tu-dortmund.de}{https://orcid.org/0000-0002-6023-8921}{}{}
...
Intersection type systems \cite{cdv81,BarendregtCD83} are important in  the theory of typed $\lambda$-calculi and programming languages \cite{BDS13}.
Such systems are highly expressive, characterizing deep semantic properties of $\lambda$-terms including normalization and solvability properties \cite{BDS13}.
Due to their enormous expressive power, the classical type theoretical decision problems, typability and inhabitation, are undecidable for such systems.
It is therefore of interest to investigate bounding principles for intersection type systems in order to obtain decidable fragments, which may be useful for applications (for example, type checking or program synthesis) or interesting for understanding the borderline between decidability and undecidability or the complexity of bounded fragments. 
 
A notion of {\em dimensional bound} was introduced by a subset of the present authors \cite{DudenhefnerR17} for intersection type derivations.
Intuitively, dimension measures the maximal cumulative width (number of components) of intersection types introduced for subterms in a derivation.
Dimension can be measured in two ways, depending on whether width is measured over sets of types or multisets of types.
The former corresponds to the standard, idempotent treatment of intersection types, the latter corresponds to a non-idempotent treatment.
Relative to each measurement system, we considered inhabitation under dimensional bound: given a type and a dimensional bound, does there exist a term having the type in the given dimension?
A key result of \cite{DudenhefnerR17} is that the dimension-bounded inhabitation problem is undecidable in set-theoretic dimension (\cite{DudenhefnerR17}, Theorem~28) but decidable and \textsf{EXPSPACE}-complete in multiset dimension (\cite{DudenhefnerR17}, Theorem~34).
For any fixed dimensional parameter, the latter problem is \textsf{PSPACE}-complete.
The main previous result concerning decidable fragments of the inhabitation problem for intersection types had been given by Urzyczyn \cite{urzy09}, showing that the problem is decidable and \textsf{EXPSPACE}-complete in rank $2$, and undecidable in all ranks $k$ for $k \ge 3$ (here referring to Leivant’s
notion of rank \cite{le83}).
Because all typings in rank $2$ are derivable in linear multiset dimension (meaning dimension $n$ for an intersection type with $n$ components~\cite[Proposition~23]{DudenhefnerR17}), the result for bounded multiset dimension represented a significant generalization of the rank $2$ borderline (typings in bounded multiset dimensions span across all ranks).

In this paper we introduce and study a system (denoted system~\textbf{R}\footnote{Daniel de~Carvalho introduced another system~\textbf{R}~\cite{Carvalho18}, which we call here for clarity system~\textbf{Rel}, where relations play a major role in its denotational semantics.}) with {\em bounded parallel intersection} providing a strict generalization of the multiset dimensional system~\cite{DudenhefnerR17}, extending (and deepening the analysis of) decidable inhabitation under dimensional bound.
Intuitively, system \textbf{R} occupies a position between the set-theoretic and the multiset dimensional systems of \cite{DudenhefnerR17}, employing a combination of intersection types represented as vectors of types (which are non-idempotent) and as sets of types (which are idempotent).
A technical novelty of the system is the use of \emph{relations} specifying type vector transformations that, intuitively, capture information flow between types in a derivation, and which mediate between vectors of types and sets of types. Dimensionality in this system is measured by the maximal length of vectors of sets of types that can be used to type a variable. Our main technical result is that inhabitation in bounded dimension is decidable in 
\textsf{2-EXPTIME}, and for each fixed dimension, inhabitation is decidable in \textsf{EXPTIME}.
We have corresponding lower bounds of \textsf{EXPSPACE} and \textsf{PSPACE}, respectively, and leave open the problem of closing the gaps.

\subparagraph*{Related work}
There are several intersection type systems with features that partially overlap with those of system~\textbf{R}.
However, none of these systems combines a syntax-directed Curry-style presentation with concise information flow management.

So-called non-idempotent intersection type systems have been shown to have decidable inhabitation \cite{BKR14}. These systems are fundamentally different from ours, since they have a~linear treatment of type assumptions. Linearity turns out to imply polynomial bounds on the depth of inhabitants (in terms of the size of inhabited types), thereby rendering the inhabitation problem much less complex (in \textsf{NP}) for such systems.

The system $\lambda^{\text{CIL}}$ by Wells et al.~\cite{WellsDMT02} (and its simplified version~\cite{WellsH02}) is a system in Church-style with explicit type annotations in $\lambda$-binders where different derivations for the same subterm are managed by means of records.
A record of types with labels from~$I$ is derived here by a record of terms of corresponding labels from $I$.
Term syntax in $\lambda^{\text{CIL}}$ is enriched with record management capabilities, which we avoid in system~\textbf{R}.

Similarly to $\lambda^{\text{CIL}}$, the system by Liquori and Ronchi Della Rocca~\cite{LiquoriR07} is a system with explicit annotations at $\lambda$-binders.
It introduces terms in which variables in binders are annotated with indices.
These indices serve as pointers to binders in derivations, and the binders in derivations contain types.
Again similarly to $\lambda^{\text{CIL}}$, term syntax is enriched with type derivation information to establish a Church-style calculus.

The proof system for \emph{Intersection Logic} presented by Pimentel, Ronchi Della Rocca, and Roversi~\cite{PimentelRR12} features independent derivations in tuples led according to the same proof rules.
Effectively, this works like an intersection type system with vectors.
However, the system does not have proof terms and relies on explicit exchange and weakening rules.
Another version of the proof system is formulated in terms of a sequent calculus~\cite{RoccaSSV10}.

The line of research on Intersection Logic~\cite{ronchi,ronchi10,PimentelRR12} inspired the development of a~vector-based sequent calculus, which we refer to as $(\vdash_{\textnormal{IL}})$, along with corresponding Kripke semantics~\cite{DudenhefnerU21} for Intersection Logic.
Proof search in $(\vdash_{\textnormal{IL}})$ corresponds to inhabitation in 
the system of Barendregt, Coppo and Dezani-Ciancaglini~\cite{BarendregtCD83}.
It is natural to compare fragments of $(\vdash_{\textnormal{IL}})$ in which the length of vectors is bounded to the presented dimensionally bounded system~\textbf{R}.
The most important difference is that in $(\vdash_{\textnormal{IL}})$ reorganization of vector coordinates in assumptions is realized via inverse images of surjective functions (see rule (R)~\cite[Figure~1]{DudenhefnerU21}).
For example, for the function $f$ such that $f(1) = 1 = f(2)$ the inverse image of the vector $(\{a,b\})$ under $f$ is $f^{-1}((\{a,b\})) = (\{a,b\}, \{a,b\})$.
As a result, information can only be duplicated or discarded, and there is no granular control over the \emph{relevant} part of assumptions.
Establishing such granular control can be realized via relations, generalizing functions, which is the exact aim of system~\textbf{R} in the present contribution.
...
If $\bar{\sigma} = (\sigma_1, \ldots, \sigma_n)$ and $\bar{A} = (A_1, \ldots, A_n)$, then $\bar{\sigma} \Rightarrow \bar{A} = (\sigma_1 \to A_1, \ldots, \sigma_n \to A_n)$, where $n\geq 0$, and in particular $()\Rightarrow()=()$. We write $\bar{A}_{[i]}$ for $A_i$ and $\bar{\sigma}_{[i]}$ for $\sigma_i$.
...
We identify $\Gamma$ and $\Gamma, x : (\emptyset, \ldots, \emptyset)$, consequently $\Gamma(y) = (\emptyset, \ldots, \emptyset)$ iff $y \not\in \dom(\Gamma)$ for any term variable $y$.
...
The original feature of the system presented in this paper is the management of flow of types between vectors by means of relations. We extend notation of previous work~\cite{DudenhefnerU21} from surjective functions to binary relations, and define type vector transformations as follows.
...
\begin{example}
For $R = \{(1, 1), (1, 3), (2, 1), (3, 3)\}$ we have $R((a, b, c)) = (\{a, b\}, \{\}, \{a, c\})$.
\end{example}
...
It also shows that the occurrence of sets of types with cardinality of more than one in derivations does not necessarily increase dimension.
...
The following Example~\ref{xmp:deduplicate} illustrates use of the above lemma to permute, remove, or copy vector coordinates in derivations.
...
Let us discuss the properties of the dimension bound with respect to different notions of reduction.
It is neither invariant under $\beta$-expansion (Example~\ref{xmp:no-beta-expansion}), $\eta$-expansion (Example~\ref{xmp:no-eta-expansion}), nor under $\eta$-reduction (Example~\ref{xmp:no-eta-reduction}).
...
\begin{example}\label{xmp:no-eta-reduction}
    ...
    However, $\emptyset \not\vdash_k \lambda x.x : (\{\{a\} \to c\} \to \{a, b\} \to c)$ for any dimension bound $k$.
\end{example}
...
Let us conclude this section by showing that for each dimension bound subject reduction with respect to $\beta$-reduction holds.
This is achieved by careful management of information flow in the proof of the following substitution lemma.
The crucial aspect of the proof is an appropriate choice of a relation in the application case.
...
\begin{proof}
    ...
    \item[Case $t = \lambda x.v$:] In this case, reduction actually has the form 
    $t=\lambda x.v\to_\beta \lambda x.v' = u$ where $v\to_\beta v'$. The only way to derive
    type for $\lambda x.v$ is to use rule ($\Rightarrow$I). This means that $\bar{B} = \bar{\sigma}\Rightarrow\bar{A}$ for some $\bar{\sigma}$ and $\bar{A}$. From that we get
    a derivation for $\Gamma, x:\bar{\sigma}\vdash_k v : \bar{A}$. We can now use the induction
    hypothesis to get $\Gamma, x:\bar{\sigma}\vdash_k v' : \bar{A}$ and then rule ($\Rightarrow$I)
    ...
    In the second case $v=\lambda x.v'$ and reduction follows the $\beta$-redex 
    ...
    \begin{displaymath}
        ...
        \DisplayProof}
    \end{displaymath}
    for some relation $R$ and vector $\bar{A}$. Then rule ($\Rightarrow$I) must be used to derive the judgment
...
\label{sec:iff_BCD}
As a technical tool to establish a relation with the traditional intersection type systems, we use the strict system of van Bakel~\cite[Section~5]{Bakel11}. This allows us to bring about a~relation with the system of Barendregt, Coppo, Dezani-Ciancaglini \cite{BarendregtCD83}, which is expressed in Theorem~\ref{theorem:bcd}.
...
\begin{proof}
    Induction on $t$. Let $n := \dim(\bar{A})$ and assume $\Gamma_{[i]}\vdash_S t:\bar{A}_{[i]}$ for each $i\in\{1,\ldots, n\}$.
     ...
     \begin{description}
        ...
        Consequently, $\bar{A}\rbar{\in}\bar{\sigma}$ and we can use rule (Ax) to get $\Gamma\vdash t:\bar{A}$.
        %
        \item[Case $t=\lambda x.u$.] If $\Gamma_{[i]}\vdash_S \lambda x.u:\bar{A}_{[i]}$ for $i\in\{1,\ldots,n\}$, then these judgments must be the result of ($\to$I) rule. Consequently, $\bar{A} = \bar{\sigma}\Rightarrow \bar{B}$ for some $\bar{\sigma}$ and $\bar{B}$ and we have derivations for $\Gamma_{[i]},x:\bar{\sigma}_{[i]}\vdash_S u:\bar{B}_{[i]}$. By the induction hypothesis, we obtain that $\Gamma, x:\bar{\sigma}\vdash u:\bar{B}$. We can now apply rule ($\Rightarrow$I) and obtain $\Gamma\vdash \lambda x.u:\bar{A}$.
        %
        \item[Case $t=u\; v$.] If  $\Gamma_{[i]}\vdash_S u\, v:\bar{A}_{[i]}$ then this can only be derived by rule ($\to$E). Consequently, we have derivations for 
        $\Gamma_{[i]}\vdash_S u : \bar{\sigma}_{[i]}\to \bar{A}_{[i]}$ and $\Gamma_{[i]}\vdash_S v:\bar{\sigma}_{[i]}$ for some $\bar{\sigma}$.
        Then the only way to derive $\Gamma_{[i]}\vdash_S v:\bar{\sigma}_{[i]}$ is to use rule ($\cap$I) so that $\Gamma_{[i]}\vdash_S v:B^j_i$ where for some $k_i$  we have $\bar{\sigma}_{[i]} = \{ B^1_i,\ldots,B^{k_i}_i\}$. 
...
\begin{proof}
    ...
    \begin{description}
        \item[Case $t=x$.] If $\Gamma\vdash x:\bar{A}$ then it can be derived only by rule (Ax). Consequently, there is $(x:\bar{\sigma}) \in\Gamma$ such that $\bar{A} \rbar{\in} \bar{\sigma}$. Consequently, we can use rule ($\cap$E) to obtain $\Gamma_{[i]}\vdash_S x:\bar{A}_{[i]}$ for $i\in\{1,\ldots, n\}$.
        %
        \item[Case $t=\lambda x.u$.] The judgment $\Gamma\vdash \lambda x.u:\bar{A}$ can be derived only by rule ($\Rightarrow$I) and thus $\bar{A}=\bar{\sigma}\Rightarrow\bar{B}$ for some $\bar{\sigma}$ and $\bar{B}$ and also $\Gamma,x:\bar{\sigma}\vdash u:\bar{B}$
        ...
        %
        \item[Case $t=u\; v$.] The judgment $\Gamma\vdash u\, v:\bar{A}$ can be derived only by rule ($\Rightarrow$E). Consequently, $\Gamma = \Gamma'\cup R(\Delta)$ for some $\Gamma', \Delta$, and $R$ such that
        ...
        %
         ...
         %
         Application of rule ($\to$E) gives $\Gamma_{[i]}\vdash_S u\, v:\bar{A}_{[i]}$ for $i\in\{1,\ldots,n\}$. \qedhere
    \end{description}
\end{proof}

An important consequence of the lemma above is that many properties of van Bakel's system are inherited. In particular, system \textbf{R} has the strong normalization property as van Bakel's system has \cite[Theorem~7.5]{Bakel04}.
...
We can also write $\Sigma\vdash_{\textnormal{BCD}} t:A$ as strict types are a special case of BCD types.
 To establish the relation between system \textbf{R} and the system BCD, we use the following statement mentioned in \cite[Section~5]{Bakel11}. 
...
\end{theorem}
A similar theorem was proved~\cite[Theorem~2.2.6]{Bakel92} for another variant of the strict system the rules of which are derivable in the strict system of van Bakel used in this paper.

Note that the formulation above omits the condition that $t$ is in $\lambda\bot$-normal form (cf.~\cite{Bakel92}).
...
The considerations above give us the following comparison of derivability in system \textbf{R} and the system BCD.
\begin{theorem} ~
    \label{theorem:bcd}
    \begin{enumerate}
...
\begin{lemma}\label{lem:apps_inversion}
    ...
    \begin{enumerate}
        \item\label{prop:apps_inversion:dims} $R_i \subseteq \{1, \ldots, \dim(\bar{A}_i)\} \times \{1, \ldots, \dim(\bar{B})\}$ is left-total and $\dim(\bar{A}_i) \leq k$ for $i \in \{1, \ldots, n\}$
...
\begin{proof}
    ...
    \begin{description}
        ...
        We obtain the claim by the induction hypothesis and repeated application of rule ($\Rightarrow$E).\qedhere
    \end{description}
\end{proof}
...
In each branch $i$, we non-deterministically guess the necessary relation $R_i$ (bounded in cardinality by $k^2$), type vector $\bar{A}_i$, environment $\Delta_i$, and recursively decide whether the judgment $\Delta_i \vdash_k u_i : \bar{A}_i$ is derivable for some $u_i$.
...
If we fix a dimension bound $k$, we obtain an \textsf{EXPTIME} inhabitation decision procedure.
...
We introduced a new syntax-directed intersection type system, which we call system~\textbf{R}, related to traditional systems \cite{BarendregtCD83,Bakel04,Bakel11}, where typing judgments assign vectors of types rather than single types. Special relations control information flow between vector components, allowing precise tracking of information flow while keeping derivations syntax-directed and manageable. Vectors also enable a natural restriction on derivations: only vectors of dimension at most $k$ are allowed, which is crucial for later results.

This system fulfills the need for a formalism in which natural bound on a~resource, here, the number of information-flow channels corresponding to vector dimension, renders inhabitation decidable. With the maximal dimension being part of the input, inhabitation is \textsf{2-EXPTIME} decidable, and fixing it as a parameter yields an \textsf{EXPTIME} decision procedure. Without a bound, inhabitation becomes undecidable \cite{urzy09}, as typings in our system coincide with ones in traditional systems. We also establish \textsf{EXPSPACE}-hardness when the maximal dimension is part of the input, leaving the exact complexity as an open problem.

An earlier system \cite{DudenhefnerR17} imposes a~similar dimensional restriction. We therefore provided a~detailed comparison highlighting both conceptual similarities and key differences in expressiveness and technical treatment, throughout which demonstrating that system \textbf{R} offers a~simpler and strictly more comprehensive framework that is easier to use in practice.
...
Another important avenue of investigation concerns type inference for system \textbf{R}.
Since the system incorporates features of both set and multiset interpretations of types, it is not immediately clear whether the typability problem falls into \textsf{NP} or \textsf{PSPACE}, as established for related systems by Dudenhefner and Rehof~\cite{DudenhefnerR17typ}.
Studying this question could provide deeper insight into the algorithmic behavior of the system.
Similarly to system \textbf{Rel} of de~Carvalho~\cite[Definition~4.1]{Carvalho18}, it is interesting whether binary relations in system~\textbf{R} can be related to term evaluation by means of a Krivine machine.
As the system $\lambda^{\text{CIL}}$ by Wells et al.~\cite{WellsDMT02} was also developed in the context of type inference problems, it would be interesting to compare it with the current system on the technical level.

As the system \textbf{Rel} of de~Carvalho uses relations in its models, it is interesting to see how the binary relations in the current system~\textbf{R} fit into this kind of denotational semantics.
%It is also interesting to see how they describe computations in the Krivine machine.

Finally, intersection type systems are often extended with subtyping, so developing a~version of system \textbf{R} with subtyping presents another interesting direction. Such an extension could potentially support subject expansion and reduction under $\eta$-reduction, but it would also complicate the algorithmic analysis of type inference and inhabitation problems, requiring careful treatment to balance expressiveness and decidability.
